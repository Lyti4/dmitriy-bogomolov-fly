...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    const unifiedItems = hardcodedPortfolioItems.map(item => ({
      ...item,
      images: item.images ? item.images : item.image ? [item.image] : [],
    }));

    // Устанавливаем состояние
    setPortfolioItems([...unifiedItems, ...wardrobeGroups]);
    setIsLoading(false);
  }, []);

  const filters = [
    { id: 'proven', name: 'Проверено в быту' },
        ...
        >
          <div className="relative max-w-5xl w-full" onClick={e => e.stopPropagation()}>
            ...
            <Splide options={{
              ...
              width: 'auto',
              height: '80vh',
              start: selectedImages.indexOf(selectedImage) >= 0 ? selectedImages.indexOf(selectedImage) : 0
            }}>
              {selectedImages.map((img: string, index: number) => (
                <SplideSlide key={`${img}-${index}`}>
                  <OptimizedImage
                    ...
                    objectFit="contain"
                    priority={index === 0} // приоритетная загрузка только для первого изображения
                  />
                </SplideSlide>
              ))}
            </Splide>
