  ...
  return (
    <section className="relative h-[600px] md:h-[700px] lg:h-[800px] overflow-hidden">
      ...
      <div className="relative h-full container mx-auto px-4 z-10 flex items-center">
        <div className="h-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center w-full">
          {/* Левая часть - триада концепции */}
          <AnimatedSection animation="slideLeft" className="text-white">
            <div className="mb-8">
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-light mb-4 drop-shadow-2xl text-black">
            ...
            <AnimatedSection animation="fadeIn" delay={800}>
              <p className="text-xl md:text-2xl lg:text-3xl drop-shadow-lg max-w-lg text-black leading-relaxed">
          ...
          >
            {[
              ...
              <div key={index} className="flex items-center space-x-3 text-black">
                <span className="text-2xl text-[#8DB892]">◼</span>
                <div>
                  <span className="font-semibold text-lg">{item.title}</span>
                  <span className="text-base opacity-90 ml-2">— {item.desc}</span>
                </div>
              </div>
            ))}
          </AnimatedSection>
      ...
      <AnimatedSection
        animation="slideUp"
        delay={1800}
        className="absolute right-4 lg:right-8 bottom-8 z-20"
      >
        <button
          onClick={handleCtaClick}
          className="group inline-flex items-center bg-white/95 hover:bg-white border-2 border-[#8DB892] text-black py-4 px-8 md:py-5 md:px-10 rounded-full transition-all duration-500 shadow-xl backdrop-blur-sm hover:shadow-2xl hover:scale-105 font-medium text-lg"
        >
          Достичь совершенства
          <svg
            className="ml-3 w-5 h-5 transform group-hover:translate-x-1 transition-transform text-[#8DB892]"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
